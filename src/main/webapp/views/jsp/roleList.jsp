               ...
               <section class="panel">
                  ...
                  <div class="panel-body">
                     <div class="adv-table editable-table ">
                        <div class="clearfix">
                           <div class="btn-group">
                              <button id="editable-sample_new" class="btn btn-success green" onclick="showAddRoleDiv()">
                        ...
                        <table class="table table-striped table-hover table-bordered" id="editable-sample">
                           <display:table id="role" name="roleList" requestURI="/system/roleList" pagesize="5" class="table">
	                        <display:setProperty name="sort.amount" value="list"></display:setProperty>
	                        <display:column property="id" title="标识" sortable="true" />
							<display:column property="name" title="角色名" sortable="true"/>
							<display:column property="code" title="角色编码" />
							<display:column property="desc" title="备注" />
							<display:column property="permissionListName" title="权限" />
							<display:column property="createTime" title="创建时间" />
							<display:column title="操作">
								<a href="javascript:void(0)" onclick="getRole(<c:out value="${role.id}"/>)">编辑</a>
								<a href="javascript:void(0)" onclick="delRole(<c:out value="${role.id}"/>)">删除</a>
							</display:column>
						</display:table>
                        </table>
                     </div>
